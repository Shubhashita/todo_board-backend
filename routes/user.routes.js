...
userRoutes.post('/login', (req, res, next) => { console.log('Login attempt for:', req.body.email); next(); }, validate(loginSchema), userController.login);
userRoutes.post('/onboard', (req, res, next) => { console.log('Signup attempt for:', req.body.email); next(); }, validate(userSchema), userController.onboard);
